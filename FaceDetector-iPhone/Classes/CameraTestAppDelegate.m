...
static int previewHeight = -1;
static int pixelDataLength = -1;
static int previewBytesPerRow = -1;
...
//	NSLog(@"[__camera_callbackHook] cameraDevice:0x%x,a:0x%x,coreSurfaceBuffer:0x%x,ID:0x%x,b:0x%x,coreSurfaceAccelerator:0x%x",
...
//		unsigned int alignment = CGBitmapGetFastestAlignment();	//orig
//		unsigned int alignmentedBytesPerRow = (width * 4 / alignment + 1) * alignment; //orig
...
static int __camera_callbackHook2(CameraDeviceRef cameraDevice,int a,CoreSurfaceBufferRef coreSurfaceBuffer,int b) {
	CoreSurfaceAcceleratorRef coreSurfaceAccelerator = *(CoreSurfaceAcceleratorRef*)(cameraDevice+84);
//	NSLog(@"[__camera_callbackHook2] cameraDevice:0x%x,a:0x%x,coreSurfaceBuffer:0x%x,b:0x%x,coreSurfaceAccelerator:0x%x",
...
//			[data writeToFile:pathToDefault atomically:NO];
			...
			copy = CGImageCreateCopy(imageRef);
...
//	for (int i = 0; i < 6; i++) {
	CoreSurfaceBufferRef coreSurfaceBuffer = *(CoreSurfaceBufferRef*)(p+88);
	Surface *surface = [[Surface alloc]initWithCoreSurfaceBuffer:coreSurfaceBuffer];
	...
	unsigned int alignment = CGBitmapGetFastestAlignment();
	unsigned int alignmentedBytesPerRow = (width * 4 / alignment + 1) * alignment;
	void *readblePixels1 = CGBitmapAllocateData(alignmentedBytesPerRow * height);
	for (unsigned int j = 0; j < height; j++) {
		memcpy(readblePixels1 + alignmentedBytesPerRow * j, pixels + bytesPerRow * j, bytesPerRow);
	}
	[surface unlock];
	
	CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
	CGDataProviderRef dataProviderRef = CGDataProviderCreateWithData(NULL, readblePixels1, alignmentedBytesPerRow * height, NULL);
	CGImageRef imageRef = CGImageCreate(width,						// size_t width,
										height,						// size_t height,
										8,							// size_t bitsPerComponent,
										32,							// size_t bitsPerPixel,
...
//		NSData *data = UIImagePNGRepresentation(image);
//		[data writeToFile:pathToDefault atomically:NO];
	CGImageRef copy = CGImageCreateCopy(imageRef);

	CGImageRelease(imageRef);
	CGDataProviderRelease(dataProviderRef);
	CGColorSpaceRelease(colorSpace);
	CGBitmapFreeData(readblePixels1);
