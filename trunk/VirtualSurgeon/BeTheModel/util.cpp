...
void detect_and_draw(cv::CascadeClassifier& cascade, cv::CascadeClassifier& nestedCascade, 
	...
	for(unsigned int i = 0; i < faces.size(); i++ )
    {
		...
		Mat small_img_roi = small_orig(*r); //Range(r->y,r->y+r->height),Range(r->x,r->x+r->width));
		...
		if(nested_objects.size() >= 2) {
			...
			for(unsigned int ii=0;ii<nested_objects_count;pairs[ii] = -1,phis[ii] = 0.0f,ii++);
...
void readCloneInfoFile(char* filename, CloneInfo& info) {
	...
	while(!i.eof()) {
		i.getline(str,256);
		info.neck_path_file = std::string(str);
		if(info.neck_path_file.size()>0) break;
	}
	i.close();
}
